...
class Solver(object):
    ...
    def train_epoch(self, epoch):
        ...
        for idx, batch in enumerate(self.train_dataset):
            ...
            if self.train_total_steps % self.opt.print_losses_freq == 0:
                ...
                print(idx * self.opt.batch_size,len(self.train_dataset))
    ...
    def test_save_imgs(self, faces_list, paths_list, re=0):
        for idx in range(len(paths_list[0])):
            ...
            if self.opt.save_test_gif:
                ...
                output_img = np.array(self.visual.numpy2im(faces_list[-2][idx]))
                output_img = Image.fromarray(output_img)
                saved_path = os.path.join(self.opt.results, "output")
                if not os.path.exists(saved_path):
                    os.makedirs(saved_path)
                saved_path = os.path.join(saved_path, "%s_%s.jpg" % (src_name, tar_name))
                output_img.save(saved_path)
